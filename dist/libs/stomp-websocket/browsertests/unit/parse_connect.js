...
(function () {
  module("Parse connect method arguments", {
    ...
    setup: function setup() {
      ...
      myConnectCallback = function myConnectCallback() {
        // called back when the client is connected to STOMP broker
      };

      myErrorCallback = function myErrorCallback() {
        // called back if the client can not connect to STOMP broker
      };

      client = Stomp.client(TEST.url);

      checkArgs = function checkArgs(args, expectedHeaders, expectedConnectCallback, expectedErrorCallback) {
        var headers = args[0];
  ...
  test("connect(login, passcode, connectCallback)", function () {
    checkArgs(client._parseConnect("jmesnil", "wombats", myConnectCallback), { login: 'jmesnil', passcode: 'wombats' }, myConnectCallback, undefined);
  });
  ...
  test("connect(headers, connectCallback)", function () {
    ...
    checkArgs(client._parseConnect(headers, myConnectCallback), headers, myConnectCallback, undefined);
  });

  test("connect(headers, connectCallback, errorCallback)", function () {
    var headers = { login: 'jmesnil', passcode: 'wombats', host: 'myvhost' };

    checkArgs(client._parseConnect(headers, myConnectCallback, myErrorCallback), headers, myConnectCallback, myErrorCallback);
  });
})();